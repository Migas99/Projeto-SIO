            ...
            validator.validateXML(data, xsdFile, function (err, result) {
                ...
                if (result.valid) {
                    ...
                    parser.parseString(data, function (err, jsonData) {
                        ...
                        AuditFile.findOne({ 'Header.FiscalYear': formatedJson.AuditFile.Header.FiscalYear }, { _id: 0 }, function (err, other) {
                            if (err) throw err;

                            if (other) {
                                return res.status(400).json({ 'Error': 'The auditfile related to that fiscal year has already been uploaded!' });
                            }

                            const auditfile = new AuditFile({
                                Header: formatedJson.AuditFile.Header,
                                MasterFiles: formatedJson.AuditFile.MasterFiles,
                                GeneralLedgerEntries: formatedJson.AuditFile.GeneralLedgerEntries,
                                SourceDocuments: formatedJson.AuditFile.SourceDocuments
                            });

                            auditfile.save();

                            return res.status(200).json({ 'Success': 'The SAFT file was uploaded with success.' });
                    ...
                    return res.status(400).json({ 'Error': 'Invalid XML file. It wasnt validated by the XSD.' })
                ...
                if (!property.includes('ID') && !property.includes('Code')) {
                    ...
                    if (typeof object[property] === 'object') {
                        ...
                        if (object.hasOwnProperty(property) && object[property] !== null && !isNaN(object[property])) {
                            ...
                            if (moment(object[property], ["DD-MM-YYYY", "YYYY-MM-DD"], true).isValid()) {
...
auditfileController.getAuditFile = async (req, res) => {
    try {
        ...
        const auditfile = await AuditFile.findOne({ 'Header.FiscalYear': parseInt(req.params.year) }, { _id: 0 });
...
auditfileController.getCompanyInfo = async (req, res) => {
    try {
        ...
        const companyInfo = await AuditFile.aggregate([
            {
                $match: { 'Header.FiscalYear': parseInt(req.params.year) }
            },
            {
                $project: {
                    _id: 0,
...
auditfileController.getAllCustomers = async (req, res) => {
    try {
        ...
        const auditfile = await AuditFile.find({ 'Header.FiscalYear': parseInt(req.params.year) });

        if (auditfile[0].MasterFiles.Customer) {

            let CustomersAnswer = [];
            const listOfCustomers = auditfile[0].MasterFiles.Customer;
...
auditfileController.getCustomerById = async (req, res) => {
    ...
    try {
        ...
        if (auditfile[0].MasterFiles.Customer) {
            ...
            const customers = auditfile[0].MasterFiles.Customer;
...
auditfileController.getAllSuppliers = async (req, res) => {
    try {
        ...
        if (auditfile[0]) {
            ...
            let SuppliersAnswer = [];
            const listOfAccounts = auditfile[0].MasterFiles.GeneralLedgerAccounts.Account;
...
auditfileController.getSupplierById = async (req, res) => {
    ...
    try {
        ...
        if (auditfile[0].MasterFiles.Supplier) {

            const suppliers = auditfile[0].MasterFiles.Supplier;
...
auditfileController.getAllProducts = async (req, res) => {
    try {
        ...
        if (auditfile[0].MasterFiles.Product) {

            let ProductsAnswer = [];
            const listOfProducts = auditfile[0].MasterFiles.Product;
...
auditfileController.getProductByCode = async (req, res) => {
    try {
        ...
        if (auditfile[0].MasterFiles.Product) {
            ...
            const products = auditfile[0].MasterFiles.Product;
...
auditfileController.getAllJournals = async (req, res) => {
    try {
        ...
        if (auditfile[0].GeneralLedgerEntries.Journal) {

            let JournalsAnswer = [];
            const listOfJournals = auditfile[0].GeneralLedgerEntries.Journal;
...
auditfileController.getJournalById = async (req, res) => {
    try {
        ...
        if (auditfile[0].GeneralLedgerEntries.Journal) {
            ...
            const journals = auditfile[0].GeneralLedgerEntries.Journal;
                ...
                for (data in listOfJournals) {
                    ...
                    if (listOfJournals[data].Transaction) {
                        ...
                        if (Array.isArray(listOfTransactions)) {
                            ...
                            for (extradata in listOfTransactions) {
                                if (listOfTransactions[extradata].TransactionID == req.body.id) {
...
auditfileController.getAllInvoices = async (req, res) => {
    ...
    try {
        ...
        if (auditfile[0].SourceDocuments.SalesInvoices.Invoice) {

            let InvoicesAnswer = [];
            const listOfInvoices = auditfile[0].SourceDocuments.SalesInvoices.Invoice;
        ...
        const salesTotalsByPeriod = await AuditFile.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    'PeriodTotal': { $sum: '$SourceDocuments.SalesInvoices.Invoice.DocumentTotals.GrossTotal' },
                    'PeriodTotalWithoutTax': { $avg: '$SourceDocuments.SalesInvoices.Invoice.DocumentTotals.GrossTotal' }
                }
            },
            {
                $sort: { '_id': 1 }
        ...
        const salesPerProduct = await AuditFile.aggregate([
            ...
            { $unwind: { "path": '$SourceDocuments.SalesInvoices.Invoice.Line.ProductCode' } },
            {
                $group: {
                    ...
                    'ProductDescription': { $addToSet: '$SourceDocuments.SalesInvoices.Invoice.Line.Description' },
    ...
    try {
        ...
        const AuditFileTotals = {
            Totals: {
                ...
                NumberOfSales: moviments[0].SourceDocuments.SalesInvoices.NumberOfEntries,
...
auditfileController.getBoughtsBySupplier = async (req, res) => {
    try {
        ...
        if (auditfile[0]) {
            ...
            const listOfBoughts = auditfile[0].GeneralLedgerEntries.Journal[0].Transaction;

            for(account in SuppliersAnswer){
                SuppliersAnswer[account].TotalBoughts = 0.0;
                var numberOfBoughts = 0;
                for(bought in listOfBoughts){
                    if(listOfBoughts[bought].Lines.CreditLine.AccountID == SuppliersAnswer[account].AccountID){
                        SuppliersAnswer[account].TotalBoughts += listOfBoughts[bought].Lines.DebitLine[1].DebitAmount;
                        numberOfBoughts += 1;
                    }
                }
                SuppliersAnswer[account].AverageBought = SuppliersAnswer[account].TotalBoughts/numberOfBoughts;
            }
